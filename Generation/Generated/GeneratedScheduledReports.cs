...
namespace TeamSupport.Data
{
  ...
  public partial class ScheduledReports : BaseCollection, IEnumerable<ScheduledReport>
  {
    ...
    public virtual void LoadById(int id)
    {
      ...
      {
        command.CommandText = "SET NOCOUNT OFF; SELECT [Id], [EmailSubject], [EmailBody], [EmailRecipients], [ReportId], [OrganizationId], [RunCount], [IsActive], [StartDate], [RecurrencyId], [Every], [Weekday], [Monthday], [LastRun], [IsSuccessful], [NextRun], [CreatorId], [ModifierId], [DateCreated], [DateModified], [LockProcessId], [FilePathID] FROM [dbo].[ScheduledReports] WHERE ([Id] = @Id);";
