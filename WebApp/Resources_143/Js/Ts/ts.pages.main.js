...
Ts.Pages.Main.prototype =
{
  ...
  init: function () {
    ...
    function processQuery() {
      ...
      if (ticketID) self.openTicketByID(ticketID, true);
      var ticketNumber = Ts.Utils.getQueryValue('ticketnumber');
      if (ticketNumber) self.openTicket(ticketNumber, true);
      ...
      switch (tab.getTabType()) {
        ...
        case Ts.Ui.Tabs.Tab.Type.Ticket:
          ...
          if (div.length < 1) {
            ...
            var nwLnk = showNewTicket ? 'Resources_143/Pages/Ticket.html?TicketNumber=' : 'frames/ticket.aspx?Embedded=1&TicketNumber=';
            $('<iframe>')
              .attr('frameborder', 0)
              .attr('scrolling', 'no')
              .appendTo(div)
              .attr('src', nwLnk + tab.getId());
            //.attr('src', 'frames/ticket.aspx?Embedded=1&TicketNumber=' + tab.getId())
          }
          else {
            div.show();
          }
          $('.main-info-content').load('Resources_143/PaneInfo/ticket.html');
        ...
        case Ts.Ui.Tabs.Tab.Type.NewTicket:
          ...
          $('.main-info-content').load('Resources_143/PaneInfo/newticket.html');
          break;
        default:
  ...
  highlightTicketTab: function (ticketNumber, isHighlighted) {
    var tab = this.MainTabs.find(ticketNumber, Ts.Ui.Tabs.Tab.Type.Ticket);
    if (tab) { tab.setIsHighlighted(isHighlighted); }
  },
