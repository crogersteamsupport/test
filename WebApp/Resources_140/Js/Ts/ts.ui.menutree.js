...
Ts.Ui.MenuTree.prototype = {
  ...
  add: function (parent, id, type, caption, imageUrl, data) {
    ...
    if (parent) {
      ...
      if (list.length < 1) {
        list = $(parent.getElement()).append('<ul></ul').children('ul');
        $(parent.getElement()).children('div').children('.ui-icon').removeClass('ts-icon-none').addClass('ui-icon-triangle-1-s').click(function (e) {
          e.stopPropagation();
    ...
    $(item.getElement()).children('div').hover(function () { $(this).addClass('ui-state-hover'); }, function () { $(this).removeClass('ui-state-hover'); });
    var self = this;
