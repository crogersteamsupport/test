...
(function ($) {
  ...
  function bindField(field) {
    ...
    var errorContainer = options.errorContainer.clone().appendTo(field.parents('div.groupBodyDiv'));
    //var errorContainer = field.parents('div.groupBodyDiv').append('<div class="ketchup-error-container"><ol></ol><span></span></div>').find('.ketchup-error-container');
  ...
  var positionContainer = function (errorContainer, field) {
    // errorContainer.animate({ top: field.offset().top - errorContainer.height() });
  };
  ...
  jQuery.validateForm = function (selector, opt) {
    validate = 'validate';
    ...
    elements.each(function () {
      ...
      for (var i = 0; i < fields.length; i++) {
        if (buildErrorList(extractValidations(fields[i].blur()), fields[i]).length) {
          result = false;
        }
      }
