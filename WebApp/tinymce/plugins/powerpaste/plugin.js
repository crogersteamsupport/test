...
define(
  ...
  function (Arr, Struct, console) {
    var fields = [ 'officeStyles', 'htmlStyles', 'isWord', 'proxyBin', 'backgroundAssets', 'isInternal' ];

    // Combine two bundles into one, with priority to the second.
    var merge = function (original, extra) {
      var r = {};
      Arr.each(fields, function (field) {
        var value = extra[field]().or(original[field]());
        value.each(function (v) { r[field] = v; });
      });
      return nu(r);
    };
...
(function (define, require, demand) {
define(
  'ephox.cement.smartpaste.PasteResponse',
  ...
  function (Option, Adt) {
    ...
    var cata = function (subject, onError, onPaste, onCancel, onIncomplete) {
      return subject.fold(onError, onPaste, onCancel, onIncomplete);
    };

    var carry = function (old, nu) {
      // At the moment, carry is just used to propogate any message from an INCOMPLETE to a subsequent
      // PASTE. In the future, it would be leveraged to do the ERROR -> ERROR and CANCEL -> CANCEL handling.
      return cata(old, Option.none, Option.none, Option.none, function (elements, assets, message) {
        return cata(nu, Option.none, function (elems, as) {
          return Option.some(adt.incomplete(elems, as, message));
...
define(
  ...
  function (Bundle, PasteResponse, FutureOps, Struct) {
    var done = Struct.immutableBag([ 'response', 'bundle' ], [ ]);
    ...
    var call = function (callback, v) {
      callback(done(v));
    };

    var pass = function (res) {
      return pure({
    ...
    var error = function (err) {
      return pure({
        response: PasteResponse.error(err),
        bundle: Bundle.nu({})
      });
    };
    ...
    var initial = function () {
      return pure({
        response: PasteResponse.paste([], []),
        bundle: Bundle.nu({})
      });
    };

    // baseFn is a function(callback) { ... }
    var sync = function(baseFn) {

      var get = function(callback) {
        // Do not detach from callstack for pasting.
        baseFn(callback);
      };

      return FutureOps(sync, get);
    };
...
define(
  ...
  function (Option, Options, RegExp) {
    var isValidData = function (data) {
      // IE doesn't even have data. Spartan has data, but no types.
      return data !== undefined && data.types !== undefined && data.types !== null;
    };

    var getPreferredFlavor = function (priority, types) {
      return Options.findMap(priority, function (p) {
        return getFlavor(types, p);
      });
    };

    var getFlavor = function (types, flavor) {
      var pmatch = new RegExp(flavor, 'i');
      return Options.findMap(types, function (t) {
        ...
        return t.match(pmatch) !== null ?
                  Option.some({ type: t, flavor: flavor }) :
                  Option.none();
      });
    };
    ...
    return {
      ...
      getPreferredFlavor: getPreferredFlavor,
      getFlavor: getFlavor
    };
  }
);
...
define(
  ...
  function (FutureStep, Bundle, Inspection, PasteResponse, Arr, Fun, Option, Options, Struct) {
    var type = Struct.immutable('steps', 'input', 'label', 'capture');

    /* The purpose of Adjudicator is:
     * 1. choose based on flavors and a fallback which pasting strategy to use to match the clipboard event
     * 2. run a particular strategy
     */

    var chooseFrom = function (flavors, raw) {
      return Options.findMap(flavors, function (flavor) {
        return flavor.getAvailable(raw).map(function (input) {
          return type(flavor.steps(), input, flavor.label(), flavor.capture());
        });
      });
    };

    var choose = function (flavors, fallback, raw) {
      var choice = chooseFrom(flavors, raw);
      return choice.getOrThunk(function () {
        return type(fallback.steps(), raw, fallback.label(), fallback.capture());
      });
    };

    // When running a series of steps, as soon as CANCEL or ERROR are encountered, the result is
    // just passed all the way through the system with no more steps being executed.
    // PASTE and INCOMPLETE, on the other hand, will just pass the values through to the next step f.
    var proceed = function (oldResult, f) {
      var pass = Fun.curry(FutureStep.pass, oldResult);

      // Execute the next step, and carry through its values into the return value. Note, this may
      // involve carrying the warning message of an INCOMPLETE into a subsequent PASTE.
      var transition = function (/* elements, assets, _message */) {
        return f().map(function (newResult) {
          var newBundle = Bundle.merge(oldResult.bundle(), newResult.bundle());
          var newResponse = PasteResponse.carry(oldResult.response(), newResult.response());
          return FutureStep.done({
      ...
      return PasteResponse.cata(oldResult.response(),
        // It is the ERROR case, so pass the value through the system without executing f
        pass,
        // It is the PASTE case, so run f (the step), accumulate the bundles, and pass through the new response
        transition,
        // It is the CANCEL case, so pass the value through the system without executing f
        pass,
        // It is the INCOMPLETE case, so run f (the step), accumulate the bundles, and carry through any INCOMPLETE message
        transition
      );
    };

    var run = function (steps, input) {
      // Remember, the return values of steps are Futures, so we want to bind the futures through the list.
      // At each stage, execute the next step (if required).
      var outcome = Arr.foldl(steps, function (rest, step) {
        return rest.bind(function (result) {
          // Run the next step using the previous result (response, bundle)
          var runNext = function () {
            return step(input, result);
          };

          // Transition from the previous result to the next step. Note, this does not have
          // to execute the next step if we have already hit CANCEL or ERROR.
          return proceed(result, runNext);
        });
      }, FutureStep.initial());

      // We don't care about the bundle for the final result, so just extract the response.
      return outcome.map(function (result) {
...
define(
  ...
  function (Adjudicator, PasteResponse, Arr, Fun, BlockTap, Event, Events, Paste) {
    return function (brokers, fallback) {
      var events = Events.create({
        cancel: Event([]),
        error: Event(['message']),
      ...
      var pasteTap = BlockTap.tap(function (nativeEvent) {
        ...
        // Decide which paste flavor to use based on the clipboard
        var decision = Adjudicator.choose(brokers, fallback, nativeEvent);
        // If the paste flavor replaces the paste event, kill the event
        if (decision.capture()) nativeEvent.preventDefault();

        // The paste tap needs to be given to the steps so that "Blocking" processes can block pasting.
        var steps = Arr.map(decision.steps(), function (step) {
          return step(pasteTap.control);
        });

        // Based on the final decision, runs the steps over the initial input.
        var pasted = Adjudicator.run(steps, decision.input());

        // The result of run is a future of PasteResponse. We need to interpret that and send it
        // through the old event API.
        pasted.get(function (response) {
          PasteResponse.cata(response, function (message) {
            events.trigger.error(message);
          }, function (elements, assets) {
            events.trigger.insert(elements, assets);
          }, function () {
            events.trigger.cancel();
          }, function (elements, assets, message) {
            // INCOMPLETE means a paste with an error message, so we trigger both events.
            events.trigger.insert(elements, assets);
            events.trigger.error(message);
          });
        });
...
(function (define, require, demand) {
define(
  'ephox.cement.api.PasteProcesses',
  ...
  function (Fun) {
    // This process is used to block what ever the control is at the start of the process
    // and unblock it at the end. In cement, it is used for blocking and unblocking paste.
    var blocking = function (process) {
      return function (control) {
        return function (rawEvent, info, response) {
          control.block();
          return process(rawEvent, info, response).map(function (next) {
            control.unblock();
            return next;
          });
        };
    ...
    // Just run the process normally, ignoring the control
    var normal = function (process) {
      return Fun.constant(process);
    };
...
(function (define, require, demand) {
define(
  'ephox.knoch.core.FutureStaticOps',
...
define(
   ...
   */
  function (Arr, Merger, Bounce, FutureOps, FutureStaticOps, Future, Option) {
...
(function (define, require, demand) {
define(
  'ephox.cement.pastiche.BackgroundAssets',
  ...
  function (IeBlob, Future, Fun, Option) {
    var background = function () {
      var assets = Option.none();

      var convert = function (raw) {
        assets = IeBlob.convert(raw);
      };

      var listen = function (callback) {
        return assets.fold(function () {
          return Future.nu(function (cb) { cb([]); });
        }, function (future) {
          return future;
        }).get(callback);
      };
...
(function (define, require, demand) {
define(
  'ephox.bowerbird.core.RtfSupport',
  ...
  function (ADT) {
    ...
    var cata = function (image, unsupported, supported) {
      return image.fold(unsupported, supported);
    };

    return {
      unsupported: adt.unsupported,
      supported: adt.supported,
      cata: cata
    };
  }
);
...
define(
  ...
  function (Fun, Option) {
    ...
    var pict = function (str, idx, start, end) {
      var bower = extractBower(str, start, end);
      // TBIO-3427. This prefix ([^a-fA-F0-9]) is added to the regex because otherwise the regex locks up the browser
      // We think it might be a pathological failure of regex.
      var hexRegex = /[^a-fA-F0-9]([a-fA-F0-9]+)\}$/;
      return common(start, end, bower, hexRegex, PICT_ID_REF);
    };

    var shp = function (str, idx, start, end) {
      var bower = extractBower(str, start, end);
    ...
    var identify = function (str, idx) {
      ...
      var pictEnd = endBracket(str, pictStart);
      var shpStart = getIndex(SHP_SIG, str, idx);
      var shpEnd = endBracket(str, shpStart);

      var idShape = Fun.curry(shp, str, idx, shpStart, shpEnd);
      var idPict = Fun.curry(pict, str, idx, pictStart, pictEnd);

      if (pictStart === -1 && shpStart === -1) return Option.none();
      else if (pictStart === -1)      return idShape();
      else if (shpStart === -1)       return idPict();

      // If we have both, and one is inside the other, return the inside one
      else if (shpStart < pictStart && shpEnd > pictEnd) return idPict();
      else if (pictStart < shpStart && pictEnd > shpEnd) return idShape();

      // Pick the first one
      else if (pictStart < shpStart)  return idPict();
      else if (shpStart < pictStart)  return idShape();
...
define(
  ...
  function (HexToBlob, RtfSupport, Species, Option, Result) {
    ...
    var extractContentType = function (pict) {
      ...
      //
      if (pict.indexOf('\\pngblip') >= 0) return Result.value('image/png');
      else if (pict.indexOf('\\jpegblip') >= 0) return Result.value('image/jpeg');
      else return Result.error('errors.imageimport.unsupported');
    };
    ...
    var extractHex = function (bower, regex) {
      var hex = bower.match(regex);
      return hex && hex[1] && hex[1].length % 2 === 0 ? Result.value(hex[1]) :
        Result.error('errors.imageimport.invalid');
    };

    var extractId = function (pict) {
      var result = pict.match(/\\shplid(\d+)/);
    ...
    var imageFromData = function (data) {
      ...
      return extractId(bower).map(
        function (id) {
          var newId = data.idRef() + id;
          return extractContentType(bower).fold(
            function (message) {
              return RtfSupport.unsupported(newId, message);
            },
            function (contentType) {
              return extractHex(bower, regex).fold(
                function (message) {
                  return RtfSupport.unsupported(newId, message);
                },
                function (hex) {
                  return RtfSupport.supported(newId, contentType, HexToBlob.convert(hex, contentType));
                }
              );
            }
          );
    ...
    var extractBowers = function (rtf) {
      ...
      var addImage = function (data) {
        ...
        images = images.concat(image.toArray());
        return data.end() + 1;
      };
    ...
    var images = function (rtf) {
      var cleaned = removeCrLf(rtf);
      return extractBowers(cleaned);
    };
...
define(
  ...
  function (Rtf, RtfSupport, Result) {
    var images = function (rtf) {
      return Rtf.images(rtf);
    };

    var toId = function (rtfSupport) {
      return RtfSupport.cata(rtfSupport, function (id, _) {
        return id;
      }, function (id, _type, _blob) {
        return id;
      });
    };

    var toBlob = function (rtfSupport) {
      return RtfSupport.cata(rtfSupport, function (_id, message) {
        return Result.error(message);
      }, function (_id, _type, blob) {
...
define(
  ...
  function (Rtf, Arr, ImageExtract, Option, Struct, Attr, Class, console) {
    var rtfData = Struct.immutable('image', 'url');



    var byCode = function (imageId, unique, _index) {
      var candidate = Arr.find(unique, function (u) {
        return Rtf.toId(u) === imageId;
      });

      return Option.from(candidate);
    };

    var byPosition = function (imageId, unique, index) {
    ...
    var uniques = function (imageData) {
      var unique = [];
      var isUnique = function (imageId) {
        return !Arr.exists(unique,
          function (data) {
            return Rtf.toId(data) === imageId;
          }
        );
      };
      return Arr.bind(imageData, function (data) {
        var id = Rtf.toId(data);
    ...
    var convert = function (images, imageData, callback, translations) {
      var unique = uniques(imageData);

      var filteredImages = [];
      var failedImage = false;

      // Search the images for IDs to use, rather than the other way around,
      // so we don't use blobs that we didn't find in the document (which causes an error to be logged by the uploader).
      var blobs = Arr.bind(images, function (image, i) {
        // TODO: Standardise the names of these data attributes. Probably namespaced.
        var type = Attr.get(image, 'data-image-type');
        var imageId = Attr.get(image, 'data-image-id');
        ...
        return converter(imageId, unique, i).fold(function () {
            console.log('WARNING: unable to find data for image ', image.dom());
            return [];
        }, function (candidate) {
          return Rtf.toBlob(candidate).fold(function (message) {
            Attr.set(image, 'alt', translations(message));
            failedImage = true;
            return [];
          }, function (blob) {
            var extractedBlob = ImageExtract.blob(blob);
            filteredImages.push(rtfData(image, extractedBlob.objurl()));
      ...
      // load the blob data before updating the src attributes and then firing the callback
      ImageExtract.fromBlobs(blobs, function (assets) {
        Arr.each(filteredImages, function (rtfData) {
          Attr.set(rtfData.image(), 'src', rtfData.url());
        });

        callback(assets, failedImage);
      });
    };
...
define(
  ...
  function (Type, Arr, Obj, Option, Attr, Body, Element, Node, Strings, Error, console, window) {
...
define(
  ...
  function (Rtf, Correlation, Arr, RtfImage, Event, Events, Remove, Traverse) {
    return function (makeFlash, cementConfig) {
      var translations = cementConfig.translations;

      var events = Events.create({
        error: Event(['message']),
        insert: Event(['elements', 'assets']),
        incomplete: Event([ 'elements', 'assets', 'message' ])
      });
      ...
      var gordon = function (content, baseAssets) {
        var response = function (event) {
          ...
          Correlation.convert(images, imageData, function (assets, failedImage) {
            if (failedImage) {
              events.trigger.incomplete(Traverse.children(content), assets.concat(baseAssets), 'errors.imageimport.failed');
            } else {
        ...
        var doIncomplete = function (message) {
          // Don't cancel the paste, but warn about why flash didn't work.
          var images = RtfImage.find(content);
          Arr.each(images, Remove.remove);
          events.trigger.incomplete(Traverse.children(content), baseAssets, message);
        };
        ...
        var onIncomplete = function (event) {
          doIncomplete(event.message());
        };

        if (cementConfig.allowLocalImages) {
          var flash = makeFlash(cementConfig);
          flash.events.response.bind(response);
          flash.events.cancel.bind(cancelImages);
          flash.events.failed.bind(onIncomplete);
          flash.events.error.bind(onIncomplete);
          flash.open();
        } else {
          doIncomplete('errors.local.images.disallowed');
        }
      };
...
define(
  ...
  function (FutureStep, Bundle, Flash, PasteResponse, RtfImage, Option, Cell, Element, InsertAll) {
    return function (makeFlash, cementConfig) {
      var flash = Flash(makeFlash, cementConfig);

      var callbacker = Cell(Option.none());

      var respond = function (response) {
        callbacker.get().each(function (cb) {
          FutureStep.call(cb, {
            response: response,
            bundle: Bundle.nu({})
          });
      ...
      flash.events.insert.bind(function (event) {
        respond(PasteResponse.paste(event.elements(), event.assets()));
      });

      flash.events.incomplete.bind(function (event) {
        respond(PasteResponse.incomplete(event.elements(), event.assets(), event.message()));
      });

      flash.events.error.bind(function (event) {
        var message = event.message();
        var response = PasteResponse.error(message);
        respond(response);
      });

      return function (raw, result) {
        return FutureStep.sync(function (callback) {
          var passthrough = function () {
            FutureStep.call(callback, {
              response: result.response(),
              bundle: result.bundle()
            });
          };

          var onPaste = function (elements, assets) {
            callbacker.set(Option.some(callback));

            var tempContainer = Element.fromTag('div');
            InsertAll.append(tempContainer, elements);

            if (RtfImage.exists(tempContainer)) flash.gordon(tempContainer, assets);
            else passthrough();
          };

          /* NOTE: No special handling of incomplete for flash. It just assumes complete */
          PasteResponse.cata(result.response(), passthrough, onPaste, passthrough, onPaste);
        });
      };
    };
  }
...
define(
  ...
  function (FutureStep, PasteResponse, Future, console) {
    var proxyBin = function (bundle) {
      var handle = function (err, f) {
        return bundle.proxyBin().fold(function () {
          console.error(err);
          return FutureStep.pure({
            response: PasteResponse.cancel(),
            bundle: {}
          });
        }, f);
    ...
    var backgroundAssets = function (bundle) {
      // Just a normal future with a list of assets.
      return Future.nu(function (callback) {
        bundle.backgroundAssets().fold(function () {
          callback([]);
        }, function (assets) {
          assets.listen(callback);
        });
      });
    };

    var merging = function (bundle) {
      var w = isWord(bundle);
      return (w && mergeOffice(bundle)) || (!w && mergeNormal(bundle));
    };

    var mergeOffice = function (bundle) {
      return bundle.officeStyles().getOr(true);
    };

    var mergeNormal = function (bundle) {
      return bundle.htmlStyles().getOr(false);
    };

    var isWord = function (bundle) {
      return bundle.isWord().getOr(false);
    };

    var isInternal = function (bundle) {
      return bundle.isInternal().getOr(false);
    };

    return {
      proxyBin: proxyBin,
      merging: merging,
      mergeOffice: mergeOffice,
      mergeNormal: mergeNormal,
...
define(
  ...
  function (Styles, Merger, Fun, Option, Class, Element, Insert) {
    return function (createDialog, settings) {
      var translations = settings.translations;

      var result = function (value, settings, callback) {
        callback(Option.some(Merger.merge(settings, {
          officeStyles: value,
          htmlStyles: value
        })));
      };
...
define(
  ...
  function (FutureStep, Bundle, Bundles, MergeSettings, PasteResponse) {
    var fromConfig = function (createDialog, config) {
      var mergeSettings = MergeSettings(createDialog, config);
      return function (rawEvent, result) {
        var bundle = result.bundle();
        var response = result.response();
        return FutureStep.sync(function (callback) {
          mergeSettings.get(Bundles.isWord(bundle), function (settings) {
            var values = settings.fold(function () {
              // They have cancelled the dialog, so cancel the paste.
              return {
                response: PasteResponse.cancel(),
                bundle: result.bundle()
              };
            }, function (set) {
              // They have chosen a merge setting.
              return {
                response: response,
                bundle: Bundle.nu({
                  officeStyles: set.officeStyles,
                  htmlStyles: set.htmlStyles
                })
              };
            });

            FutureStep.call(callback, values);
          });
        });
      };
    };

    // When doing an internal paste, we do not want the merge settings to come up. Therefore,
    // if we detect that it is internal, we just pass through "TRUE". Otherwise, we care
    // about the config.
    var fromConfigIfExternal = function (createDialog, config) {
      return function (rawEvent, result) {
        if (Bundles.isInternal(result.bundle())) {
          return FutureStep.pure({
            response: result.response(),
            bundle: Bundle.nu({
              officeStyles: true,
        ...
        else return fromConfig(createDialog, config)(rawEvent, result);
      };
    };

    var fixed = function (officeStyles, htmlStyles) {
      return function (rawEvent, result) {
        return FutureStep.pure({
          response: result.response(),
          bundle: Bundle.nu({
    ...
    return {
      ...
      fromConfigIfExternal: fromConfigIfExternal
    };
  }
);
})(ephox.bolt.module.api.define, ephox.bolt.module.api.require, ephox.bolt.module.api.demand);
...
(function (define, require, demand) {
define(
  'ephox.cement.html.ImageReference',
...
(function (define, require, demand) {
define(
  'ephox.cement.html.VshapeCorrelation',
  ...
  function (Type, Arr, Fun, Option, Attr, Class) {
    ...
    var tagAsRtf = function (img, vshapeAttrs) {
    ...
    var matches = function (attribute, identifier, imgData) {
      return imgData.img()[attribute] === identifier;
    };

    var findInSources = function (sourceList, target, attribute) {
      var identifier = Attr.get(target, attribute);
      var matcher = Fun.curry(matches, attribute, identifier);
      var raw = Arr.find(sourceList, matcher);
      return Option.from(raw).map(function (r) {
        Attr.remove(target, attribute);
        return r;
      });
    };

    var insertRtfCorrelation = function (sourceList, tordImages, attribute) {
      Arr.each(tordImages, function (img) {
        findInSources(sourceList, img, attribute).each(function (candidate) {
          tagAsRtf(img, candidate.vshape());
        });
      });
    };
    ...
    return {
      ...
      insertRtfCorrelation: insertRtfCorrelation
    };
  }
);
})(ephox.bolt.module.api.define, ephox.bolt.module.api.require, ephox.bolt.module.api.demand);
...
define(
  ...
  function (Arr, Struct) {
    var ruleStruct = Struct.immutable('selector', 'style', 'raw');
    ...
    var extract = function (stylesheet) {
      ...
      return Arr.map(domRules, function (rule) {
        ...
        return ruleStruct(selector, style, rule.style);
      });
    };
...
(function (define, require, demand) {
define(
  'ephox.cement.html.TordPostprocess',
  ...
  function (VshapeCorrelation, Arr, Fun, Attr, Comment, Css, Remove, SelectorFilter, Traverse, DocStyle, Rules) {
    var process = function (doc, container, sourceList, attribute, strategy) {
      var images = SelectorFilter.descendants(container, 'img');

      removeFragmentComments(container);
      VshapeCorrelation.insertRtfCorrelation(sourceList, images, attribute);
      var mergeStyles = strategy.mergeInline() ? mergeInlineStyles : noMerge;
      mergeStyles(doc, container);
    };

    var noMerge = Fun.noop;

    // Consider moving into vogue.
    var getCss = function (rawStyle) {
      var css = {};
      Arr.each(rawStyle, function (key) {
        // NOTE: Firefox will give us "undefined" for some styles.
        if (rawStyle[key] !== undefined) css[key] = rawStyle[key];
      });
      return css;
    };

    // NOTE: This won't handle specific or css rules. We would need to sort the rules first
    // to do that.
    var doMergeInlineStyles = function (container, rules) {
      // record the elements to remove the class from
      var elems = Arr.bind(rules, function (rule) {
        var matchingElements = SelectorFilter.descendants(container, rule.selector());

        Arr.each(matchingElements, function (element) {
          var css = getCss(rule.raw());
          Css.setAll(element, css);
        });
      ...
      // Remove all classes.
      Arr.each(elems, function (elem) {
        Attr.remove(elem, 'class');
      });
    };

    var mergeInlineStyles = function (doc, body) {
      var stylesheets = DocStyle.stylesheets(doc);
      var rules = Rules.extractAll(stylesheets);
      doMergeInlineStyles(body, rules);
    };

    var removeFragmentComments = function (container) {
      var children = Traverse.children(container);
      Arr.each(children, function (c) {
    ...
    return {
      doMergeInlineStyles: doMergeInlineStyles,
      process: process
    };
  }
);
...
define(
  ...
  function (FutureStep, Bundle, ImageReference, TordPostprocess, PasteResponse, Fun, Sandbox, Element, Elements, Html, document) {
    ...
    var tordPostProcessor = function (sourceImageList, mergeStyles) {
      ...
      return function (dump, callback) {
        ...
        sandbox.play(dump, function (iframeDoc, body) {
          TordPostprocess.process(iframeDoc, body, sourceImageList, flagAttribute, {
            mergeInline: Fun.constant(mergeStyles)
          });
    ...
    var runPaste = function (safeHtml, mergeStyles, dump) {
      return FutureStep.sync(function (callback) {
        ...
        postProcess(dump, function (tordHtml) {
          ...
          FutureStep.call(callback, {
            response: PasteResponse.paste(elements, []),
            bundle: Bundle.nu({})
          });
        });
      });
    ...
    var emptyPaste = function () {
      return FutureStep.pure(PasteResponse.paste([], []), {});
    };
    ...
    var handle = function (data, mergeStyles, cleaner) {
      var html = cleanEnd(data);

      var safeHtml = stripImageSources(html);

      return cleaner.cleanDocument(safeHtml, mergeStyles).fold(function () {
        return FutureStep.pure({
          response: PasteResponse.error('errors.paste.word.notready'),
          bundle: {}
        });
      }, function (dump) {
        return emptyString(dump) ? emptyPaste() : runPaste(safeHtml, mergeStyles, dump);
      });
    };
...
(function (define, require, demand) {
define(
  'ephox.cement.images.ImageHandler',
  ...
  function (FutureStep, Bundle, PasteResponse, Arr, ImageAsset, ImageExtract, Attr, Element, console) {
    ...
    var handle = function (items) {
      var images = Arr.filter(items, function (item) {
      ...
      return FutureStep.sync(function (callback) {
        ImageExtract.toAssets(files, function (assets) {
          ...
          FutureStep.call(callback, {
            response: PasteResponse.paste(elements, assets),
            bundle: Bundle.nu({})
          });
        });
      });
...
define(
  ...
  function (Token) {
    var create = function (doc) {
      ...
      var pop = function() {
        currentNode = currentNode.parentNode;
        if (currentNode === null) currentNode = initialNode;
      };
...
define(
  ...
  function (Arr, Token, TokenUtil, ListGuess, Fun) {
    // TBIO-2982: Office 2016 looks like it can copy across headings as lists
    var validTags = [  'P' ];

    var isInvalid = function (token) {
      return (!Arr.contains(validTags, token.tag()) || /^MsoHeading/.test(TokenUtil.getAttribute(token, 'class')));
    };
    ...
    var guess = function(bulletInfo, preferredType, originalToken) {
      ...
      return candidate.fold(function () {
        ...
        if (c.tag === 'OL' && originalToken && isInvalid(originalToken)) {
          // Don't convert numbered headings but do convert bulleted headings.
          listType = null;
        } else {
          return c;
...
define(
  ...
  function (Token) {
    ...
    var is = function (state, token) {
      // TBIO-2982: Office 2016 (in combination with textboxio/example.css on mchammer potentially) is using strongs as markers.
      return token.tag() === 'A' || token.tag() === 'SPAN' || token.tag() === 'STRONG';
    };
...
define(
  ...
  function (TokenUtil, Browser, Comments, Microsoft, Tags, ListDetect, ListTypes, Option) {
  ...
  function (HtmlPatterns, HybridAction, RuleConditions, RuleMutations, Lists, Fun, Class, Css, StringMatch) {
    ...
    var removeWordAttributes = HybridAction.removal({
      attributes: [
        ...
        // NOTE: We used to remove the "align" attribute here, but our test data is expecting it.
        // Given that previously we weren't including these filters at all, we have decided to 
        // not remove the align attribute. We may need to revisit if users would prefer it removed.
...
define(
  ...
  function (Arr, RtfImage, Strategies, HybridAction, Filter, Token, Element) {
    ...
    var word = function (isWord, merging, platform) {
      ...
      return base.concat(lists).concat([ Strategies.removeWordAttributes ]);
    };
...
define(
  ...
  function (Arr, PasteFilters, Preprocessor, HybridAction, Html, Traverse) {
    var preprocess = function (container) {
      var children = Traverse.children(container);
...
(function (define, require, demand) {
define(
  'ephox.cement.pastiche.PasteTransfer',
  ...
  function (FutureStep, Bundle, PasteResponse, PlatformDetection, RunPaste, Result, Elements, console) {
    var platform = PlatformDetection.detect();

    var runFilters = function (doc, container, merging, isWord) {
      try {
        var dump = RunPaste.go(doc, platform, container, merging, isWord);
        var isValid = dump !== undefined && dump !== null && dump.length > 0;
        var elements = isValid ? Elements.fromHtml(dump) : [ ];
        return Result.value(elements);
      } catch (e) {
        console.error(e);
        return Result.error('errors.paste.process.failure');
      }
    };

    var transfer = function (doc, container, isWord, merging, backgroundAssets) {
      var elements = runFilters(doc, container, merging, isWord);
      return elements.fold(function (message) {
        return FutureStep.error(message);
      }, function (elems) {
        return FutureStep.sync(function (callback) {
          backgroundAssets.get(function (assets) {
            FutureStep.call(callback, {
              response: PasteResponse.paste(elems, assets),
              bundle: Bundle.nu({})
            });
          });
        });
...
(function (define, require, demand) {
define(
  'ephox.cement.pastiche.OffscreenHandler',
  ...
  function (PasteTransfer, Arr, Future, Option, Remove, Traverse) {
    var external = function (doc, proxyBin, merging, isWord, backgroundAssets) {
      return PasteTransfer.transfer(doc, proxyBin, isWord, merging, backgroundAssets);
    };

    var internal = function (doc, intraFlag, proxyBin) {
      // NOTE: Internal should not include word content.
      var IS_WORD = false;
      var MERGING = true;
      ...
      var tags = findClipboardTags(proxyBin, IS_WORD).getOr([]);
      Arr.each(tags, Remove.remove);

      var noAssets = Future.nu(function (callback) { callback([]); });
      return PasteTransfer.transfer(doc, proxyBin, IS_WORD, MERGING, noAssets);
    };
...
define(
  ...
  function (Arr, Element, Html, TextContent) {
    ...
    var encode = function (text) {
      ...
      TextContent.set(wrapper, text);
      return Html.get(wrapper);
    };

    // Converts a string with newline characters into HTML
    var convert = function (text) {
      var blocks = text.trim().split(/\n{2,}|(?:\r\n){2,}/); // two or more newlines == paragraph separator

      var paragraphs = Arr.map(blocks, function (p) {
        return p.split(/\n|\r\n/).join('<br />'); // single newline == BR tag
      });

      // TBIO can handle merging text with only one paragraph, but best to be safe
      return paragraphs.length  === 1 ? paragraphs[0] : Arr.map(paragraphs, function (p) { return '<p>' + p + '</p>'; }).join('');
    };
...
(function (define, require, demand) {
define(
  'ephox.cement.plain.TextHandler',
  ...
  function (FutureStep, TextToHtml, Bundle, Inspection, PasteResponse, Option, Options, Elements, window) {
    // We don't want to trim here, because a single space character should be considered content and not empty.
    var notEmptyString = function (s) { return s.length > 0; };

    var getHtml5Text = function (data) {
      if (!Inspection.isValidData(data)) return Option.none();
      return Options.findMap(data.types, function (t) {
        return (t === 'text/plain') ? Option.some(data.getData(t)) : Option.none();
      });
    };

    var getIeText = function () {
      var ieData = window.clipboardData;
      return ieData !== undefined ? Option.some(ieData.getData('text')) : Option.none();
    };

    var pasteText = function (rawtext) {
      var text = TextToHtml.encode(rawtext);
      var html = TextToHtml.convert(text);
      var elements = Elements.fromHtml(html);
      return PasteResponse.paste(elements, [ ]);
    };

    var handle = function (data) {
      return FutureStep.sync(function (callback) {
        var textLoader = Inspection.isValidData(data) ? getHtml5Text : getIeText;
        var response = textLoader(data).filter(notEmptyString).fold(PasteResponse.cancel, pasteText);
        FutureStep.call(callback, {
          response: response,
          bundle: Bundle.nu({})
        });
...
define(
  ...
  function (FutureStep, Bundle, Bundles, HtmlHandler, ImageHandler, OffscreenHandler, TextHandler, PasteResponse, PlatformDetection, RunPaste, Fun, Element, InsertAll, Traverse, console) {
    var platform = PlatformDetection.detect();

    var plain = function (clipboardData, _response, _bundle) {
      return TextHandler.handle(clipboardData);
    };

    var autolink = function (_raw, result) {
      var onPaste = function (elements, assets) {
        var container = Element.fromTag('div');
        InsertAll.append(container, elements);
        RunPaste.preprocess(container);
        var linked = Traverse.children(container);
        return FutureStep.pure({
          response: PasteResponse.paste(linked, assets),
          bundle: result.bundle()
        });
      };

      var passthrough = Fun.curry(FutureStep.pass, result);
      return PasteResponse.cata(result.response(), passthrough, onPaste, passthrough, onPaste);
    };

    var noImages = function (_info, _result) {
      return FutureStep.error('errors.local.images.disallowed');
    };

    // Internal pasting is used for copying tables from within our editor.
    var internal = function (body, intraFlag) {
      return function (_raw, result) {
        var bundle = result.bundle();
        return Bundles.proxyBin(bundle).handle(
          'There was no proxy bin setup. Ensure you have run proxyStep first.',
          function (proxyBin) {
            var doc = Traverse.owner(body);
            return OffscreenHandler.internal(doc, intraFlag, proxyBin);
          }
        );
      };
    };

    // External pasting is "pastiche"
    var external = function (body, intraFlag) {
      return function (raw, result) {
        var bundle = result.bundle();
        return Bundles.proxyBin(bundle).handle(
          'There was no proxy bin setup. Ensure you have run proxyStep first.',
          function (proxyBin) {
            var merging = Bundles.merging(bundle);
            var isWord = Bundles.isWord(bundle);
            var isInternal = Bundles.isInternal(bundle);
            var backgroundAssets = Bundles.backgroundAssets(bundle);
            var doc = Traverse.owner(body);
            // It turns out that when copying and pasting from another browser, the custom clipboard types
            // are not found on the clipboard, so it won't go through the internal path. This means
            // that we still need to redirect it to the internal paste if the flags have been detected
            return isInternal ? OffscreenHandler.internal(doc, intraFlag, proxyBin) : OffscreenHandler.external(doc, proxyBin, merging, isWord, backgroundAssets);
          }
        );
      };
    };

    // Image pasting is only used in Webkit, and is forbidden in Safari.
    var images = function (items, _result) {
      if (platform.browser.isSafari()) return FutureStep.error('safari.imagepaste');
      else return ImageHandler.handle(items);
    };

    // The GWT pasting that EditLive! uses. Cleaner can be used to mock out that component.
    var gwt = function (cleaner) {
      return function (data, result) {
        var officeStyles = Bundles.mergeOffice(result.bundle());
        return HtmlHandler.handle(data, officeStyles, cleaner);
      };
    };

    // A fallback option if no paste can be found.
    var none = function (_data, _result) {
      return FutureStep.cancel();
    };

    // Used for changing the bundle in between steps.
    var setBundle = function (values) {
      return function (_data, result) {
        var newBundle = Bundle.merge(result.bundle(), Bundle.nu(values));
        return FutureStep.pure({
          response: result.response(),
          bundle: newBundle
        });
...
define(
  ...
  function (FutureStep, Bundle, CementConstants, PlatformDetection, Sources, Class, Traverse) {
    var platform = PlatformDetection.detect();

    return function (makeProxy, prePasteFilter, getAssets, body, intraFlag) {
      return function (raw, result) {
        // We need to read any assets from the clipboard before we fire the asynchronous
        // offscreen paste.
        var assets = getAssets();
        var response = result.response();
        return FutureStep.sync(function (callback) {
          // The makeProxy function can be used to generate a stub/mock for testing.
          var proxyPaste = makeProxy(body);

          proxyPaste.events.after.bind(function (event) {
            var proxyBin = event.container();

            // Run a paste filter over the off-screen div.
            prePasteFilter(proxyBin);

            Class.add(proxyBin, CementConstants.binStyle());

            var isWord = Sources.isWord(platform, proxyBin);
            var children = Traverse.children(proxyBin);

            // Check whether or not the content contains the magic flag that says "copied from textbox"
            var isInternal = intraFlag.findClipboardTags(children, isWord).isSome();

            // INVESTIGATE: Should we destroy the proxy here. At the moment (2015/09/15), it is a noop anyway.
            FutureStep.call(callback, {
              response: response,
              bundle: Bundle.nu({
                isWord: isWord,
...
define(
  ...
  function (PasteProcesses, BackgroundAssets, Inspection, FlashStep, MergeStep, PasteSteps, ProxyStep, PlatformDetection, Fun, Option, Strings, window) {
    var platform = PlatformDetection.detect();

    // Both flavors are supported for image copy.
    var IMAGE_FLAVOR = 'image';
    var FILE_FLAVOR = 'file';
    var IMAGE_PRIORITIES = [ IMAGE_FLAVOR, FILE_FLAVOR ];

    // NOTE: Matches XHTML as well as HTML
    var HTML_FLAVOR = 'html';


    // We can only do GWT Paste if this is true.
    ...
    // For GWT, we need to support HTML, and the HTML on the clipboard needs to contain Microsoft namespaces.
    var getGwt = function (raw) {
      var clipboardData = raw.clipboardData;
      if (! Inspection.isValidData(clipboardData)) return Option.none();
      return Inspection.getFlavor(clipboardData.types, HTML_FLAVOR).bind(function (info) {
        var data = clipboardData.getData(info.type);
        return dataContainsMicrosoftOfficeUrn(data) ? Option.some(data) : Option.none();
      });
    };

    // For Image, we need to be on Webkit/Blink and support FILE or IMAGE
    var getImage = function (raw) {
      // NOTE: IE and Firefox handle image copying and pasting themselves.
      if (platform.browser.isIE() || platform.browser.isFirefox()) return Option.none();
      else if (! Inspection.isValidData(raw.clipboardData)) return Option.none();
      else {
        var clipboardData = raw.clipboardData;
        return Inspection.getPreferredFlavor(IMAGE_PRIORITIES, clipboardData.types).map(function (info) {
          return clipboardData.items;
        });
      }
    };

    // NOTE: This flavour is only for copying and pasting tables from Textbox.io when within the
    // same browser. The external path can still use an internal paste, but it will have to do it
    // via content detection, whereas this path is based on the data type being on the clipboard.
    var getInternal = function (intraFlag, raw) {
      var dataTypes = Inspection.isValidData(raw.clipboardData) ? raw.clipboardData.types : [ ];
      return Inspection.getFlavor(dataTypes, intraFlag.clipboardType()).map(function (info) {
        return raw;
      });
    };

    // Plain text copy
    var getText = function (raw) {
      if (platform.browser.isIE()) return Option.some(window.clipboardData);
      else return Inspection.isValidData(raw.clipboardData) ? Option.some(raw.clipboardData) : Option.none();
    };

    var strategy = function (label, getAvailable, steps, capture) {
      return {
        label: Fun.constant(label),
        getAvailable: getAvailable,
        steps: Fun.constant(steps),
        capture: Fun.constant(capture)
      };
    };

    var internal = function (prePasteFilter, makeProxy, body, intraFlag) {
      var getAvailable = Fun.curry(getInternal, intraFlag);
      return strategy('Within Textbox.io (tables) pasting', getAvailable, [
        PasteProcesses.normal(ProxyStep(makeProxy, prePasteFilter, BackgroundAssets.ignore, body, intraFlag)),
        PasteProcesses.normal(MergeStep.fixed(true, true)),
        PasteProcesses.normal(PasteSteps.internal(body, intraFlag))
      ], false);
    };

    var pastiche = function (createDialog, config, prePasteFilter, makeProxy, body, makeFlash, intraFlag) {
      return strategy('Outside of Textbox.io pasting (could be internal but another browser)', Option.some, [
        PasteProcesses.normal(ProxyStep(makeProxy, prePasteFilter, BackgroundAssets.background, body, intraFlag)),
        PasteProcesses.normal(MergeStep.fromConfigIfExternal(createDialog, config)),
        PasteProcesses.normal(PasteSteps.external(body, intraFlag)),
        PasteProcesses.blocking(FlashStep(makeFlash, config))
      ], false);
    };

    var gwt = function (cleaner, createDialog, config, makeFlash) {
      return strategy('GWT pasting', getGwt, [
        PasteProcesses.normal(PasteSteps.setBundle({ isWord: true })),
        PasteProcesses.normal(MergeStep.fromConfig(createDialog, config)),
        PasteProcesses.normal(PasteSteps.gwt(cleaner)),
        PasteProcesses.blocking(FlashStep(makeFlash, config))
      ], true);
    };

    var image = function (config) {
      return strategy('Image pasting', getImage, [
        PasteProcesses.normal(config.allowLocalImages === false ? PasteSteps.noImages : PasteSteps.images)
      ], true);
    };

    var text = function () {
      return strategy('Plain text pasting', getText, [
        PasteProcesses.normal(PasteSteps.plain),
        // We may want to make this configurable in the future.
        PasteProcesses.normal(PasteSteps.autolink)
      ], true);
    };

    var none = function () {
      return strategy('There is no valid way to paste', Option.some, [
        PasteProcesses.normal(PasteSteps.none)
      ], false);
    };
...
define(
  ...
  function (Styles, PlatformDetection, Attr, Class, Element, Html, InsertAll) {
    ...
    var indicator = function (translations) {
      ...
      var waitLabel = translations('loading.wait');
      Html.set(loadNote, waitLabel);
      Attr.set(loadNote, 'aria-label', waitLabel);
...
(function (define, require, demand) {
define(
  'ephox.cement.alien.CrashMonitor',
  ...
  function (Option, Event, Events, Cell, clearInterval, setInterval) {
    var responsive = function () {
      ...
      var start = function (interval, numChecks, hasCrashed) {
        var num = numChecks;
        var t = setInterval(function () {
          if (num <= 0) {
            ...
            clearInterval(t);
            events.trigger.crashed();
          }
          num--;
        }, interval);
        timer.set(Option.some(t));
      };
...
define(
  ...
  function (CrashMonitor, WaitForFlash, Styles, Arr, Obj, Id, PlatformDetection, Globaliser, Option, Event, Events, Class, Css, Element, Focus, Insert, clearInterval, clearTimeout, console, setInterval, setTimeout, unescape, window) {
    ...
    return function (swfLoc) {
      ...
      var monitor = CrashMonitor.responsive();
      monitor.events.crashed.bind(function () {
        events.trigger.failed('flash.crashed');
      });
      monitor.events.timeout.bind(function () {
        events.trigger.failed('flash.crashed');
      });
      ...
      var loadComplete = function () {
        ...
        try {
          ...
          Obj.each(flashFunctionMapping, function (v, k) {
            var f = dom[k];
            if (f === undefined) throw 'Flash object does not have the method "' + k + '"';
          ...
          console.log('Flash dialog - Error during load: ', e);
        }
      };
      ...
      var initPaste = function () {
        monitor.start(1000, 10, function () {
          return !bin.dom().setVariable;
        });
        events.trigger.spin();
      };

      var flashFunctionMapping = {
        'setSpinCallback': globaliser.permanent(initPaste),
        ...
        'setStartPasteCallback': globaliser.permanent(stopMonitor)
      };
  ...
  function (FlashInfo, Flashbin, Event, Events, DomEvent, Element, window) {
    ...
    return function (createDialog, settings) {
      ...
      var open = function () {
        ...
        // If Flash crashes or times out
        bin.events.failed.bind(function (event) {
          hide();
          events.trigger.failed(event.message());
        });
...
define(
  ...
  function (Arr, SelectionRange, Situ, Node) {
    var beforeSpecial = function (element, offset) {
      ...
      if (!Arr.contains([ 'br', 'img' ], Node.name(element))) return Situ.on(element, offset);
      else return offset === 0 ? Situ.before(element) : Situ.after(element);
    };

    var preprocess = function (selection) {
      var start = selection.start().fold(Situ.before, beforeSpecial, Situ.after);
      var finish = selection.finish().fold(Situ.before, beforeSpecial, Situ.after);
...
define(
  ...
  function (SelectionRange, Situ, Supported, Within, DomRange, WwwcModel, Compare, Element) {
    ...
    var bounds = function (win, element) {
      return Supported.run(win, WwwcModel.bounds(element));
    };
...
define(
  ...
  function (PlatformDetection, WindowSelection, Event, Events, BodySwitch, DivSwitch, ProxyPaste, Node) {
    var platform = PlatformDetection.detect();
    ...
    // so inserting into offscreen div fails, bypassing the rest of the paste process.
    ...
    var choose = function (element) {
      var switchF = platform.browser.isIE() && Node.name(element) !== 'body' ? DivSwitch: BodySwitch;
      return switchF(fakeSelection);
    };

    return function (body) {
      var events = Events.create({
        after: Event([ 'container' ])
      });

      var switcher = choose(body);
      var proxyPaste = ProxyPaste(switcher, body);

      proxyPaste.events.after.bind(function (event) {
        switcher.toOn(body, event.container());
        events.trigger.after(event.container());
      });
...
define(
  ...
  function (PasteBroker, PasteFlavors, FlashDialog, Proxy, Arr, Id, WordCleaner, Fun, Option) {
    return function (body, createDialog, prePasteFilter, cementConfig) {
      var cleaner = WordCleaner(cementConfig.baseUrl);

      var makeFlash = Fun.curry(FlashDialog, createDialog);
      var makeProxy = Proxy;

      var noInternal = function () {
        return {
          clipboardType: Id.generate('clipboard-type'),
          findClipboardTags: Option.none
        };
      };

      var intraFlag = cementConfig.intraFlag !== undefined ? cementConfig.intraFlag : noInternal();

      var brokers = Arr.flatten([
        // Only use the internal paste if it is supported. Note, it is not yet supported by Tiny.
        cementConfig.intraFlag !== undefined ? PasteFlavors.internal(prePasteFilter, makeProxy, body, intraFlag) : [],
        [ PasteFlavors.gwt(cleaner, createDialog, cementConfig, makeFlash) ],
        [ PasteFlavors.image(cementConfig) ]
      ]);

      var fallback = PasteFlavors.pastiche(createDialog, cementConfig, prePasteFilter, makeProxy, body, makeFlash, intraFlag);

      return PasteBroker(brokers, fallback);
    };
  }
);
})(ephox.bolt.module.api.define, ephox.bolt.module.api.require, ephox.bolt.module.api.demand);
    ...
    return function (editor, url, settings, uploader) {
      ...
      editor.on('init', function(e) {
        ...
        cement.events.error.bind(function(event) {
          ...
          if (editor.notificationManager) {
            editor.notificationManager.open({
              text: I18n.translate(event.message()),
              type: 'error'
            });
          } else {
        ...
        DomEvent.bind(ed, 'paste', function (e) {
          ...
          //IE appears to require that we blur the iframe
          setTimeout(function() {
            if (editor.windowManager.windows[0] && editor.windowManager.windows[0].getEl()) {
