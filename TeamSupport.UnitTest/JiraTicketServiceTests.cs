...
namespace TeamSupport.UnitTest
{
    ...
    public class JiraTicketServiceTests
    {
        ...
        [Test]
        public void JiraTicketsService_SaveShouldSaveSuccessfully_WithMockObject()
        {
            var mockedJiraRepo = new Mock<IJiraRepository>();
             var jiraTicketService = new JiraTicketsService(mockedJiraRepo.Object);
            //Assert
            Assert.DoesNotThrow(() => jiraTicketService.SaveJiraTickets(mockedTicket.Object));
        }

        [Test]
        public void JiraTicketsService_SaveAsyncShouldSaveSuccessfullyUsingMockedRepo_WithValidEntity()
        {
            //Arrange
            var jiraTicket = new Mock<TicketLinkToJira>().Object;
            jiraTicket.id = -1;
            var mockedGenericRepo = new Mock<IGenericRepository<TicketLinkToJira>>();
            var jiraTicketService = new JiraTicketsService(new JiraRepository(mockedGenericRepo.Object));

            //Assert
            Assert.DoesNotThrow(() => jiraTicketService.SaveJiraTicketsAsync(jiraTicket));
        }
